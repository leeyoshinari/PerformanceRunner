...
def subscribe():
    try:
        groups_info = settings.REDIS.xinfo_groups(name=settings.PERFORMANCE_MESSAGE_STREAM)
        if not groups_info:
            settings.REDIS.xgroup_create(name=settings.PERFORMANCE_MESSAGE_STREAM, groupname=settings.PERFORMANCE_CONSUMER_GROUP, mkstream=True)
    except redis.exceptions.ResponseError:
        settings.REDIS.xgroup_create(name=settings.PERFORMANCE_MESSAGE_STREAM, groupname=settings.PERFORMANCE_CONSUMER_GROUP, mkstream=True)
    while True:
        try:
            messages = settings.REDIS.xreadgroup(groupname=settings.PERFORMANCE_CONSUMER_GROUP, consumername=settings.IP,
                                                 streams={settings.PERFORMANCE_MESSAGE_STREAM: '>'}, count=1, block=0)
            for message in messages:
                for data in message[-1]:
                    logger.info(data[1]['data'])
                    set_message(json.loads(data[1]['data']))
                    settings.REDIS.xack(settings.PERFORMANCE_MESSAGE_STREAM, settings.IP, data[0])
                    settings.REDIS.xdel(settings.PERFORMANCE_MESSAGE_STREAM, data[0])
        except:
        ...
        try:
            ...
            if plans.is_valid == 'true':
                ...
                if len(thread_groups) == 1:
                    num_threads = plans.target_num + 20 if plans.type == 1 else plans.target_num
                    num_threads = min(num_threads, 200)
                    ramp_time = int(num_threads / 3) if plans.type == 1 else thread_groups[0].ramp_time
    ...
    try:
        ...
        if host:
            ...
            if host_info and host_info['status'] == 0:
                post_data['host'] = [host]
                settings.REDIS.publish(settings.PERFORMANCE_MESSAGE_KEY, json.dumps(post_data, ensure_ascii=False))
            ...
            available_host = [h['host'] for h in available_servers]
            post_data['host'] = available_host[: tasks.plan.server_number]
            settings.REDIS.publish(settings.PERFORMANCE_MESSAGE_KEY, json.dumps(post_data, ensure_ascii=False))
...
def stop_task(request):
    if request.method == 'GET':
        ...
        return stop_test(task_id, host, username, ip)


def stop_test(task_id, host, username, ip):
    try:
        if host == 'all':
...
def change_tps(request):
    if request.method == 'POST':
        ...
        host = request.POST.get('host')
        return change_tps_test(task_id, host, tps, username, ip)


def change_tps_test(task_id, host, tps, username, ip):
    try:
        try:
            tps = float(tps)
        except ValueError:
        ...
        post_data = {'host': hosts, 'taskId': task_id, 'tps': current_tps, 'cmd': 'changeTPS'}
        settings.REDIS.publish(settings.PERFORMANCE_MESSAGE_KEY, json.dumps(post_data, ensure_ascii=False))
        ...
        return result(msg='TPS is changing, pls wait a minute ~')
    except:
        logger.error(traceback.format_exc())
...
def set_message(data):
    try:
        ...
        tasks.save()
        logger.info(f'Set message success, type:{task_type}, task ID: {task_id}, host: {host}')
    except:
        logger.error(traceback.format_exc())
